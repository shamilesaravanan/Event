...
def reverse_tuple_string(location_string):
    # Check if the location string is None or empty
    if not location_string:
        return location_string  # Return the original value if it's None or an empty string
    
    # Proceed with splitting if the location_string is valid
    args = location_string.split(",")
    if len(args) < 2:
        # Handle cases where the string is not formatted correctly
        return location_string  # You can choose how to handle this (e.g., return an error message or the original string)
